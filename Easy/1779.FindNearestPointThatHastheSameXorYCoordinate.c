...
int nearestValidPoint(int x, int y, int **points, int pointsSize, int *pointsColSize)
{
  ...
  for (int i = 0; i < pointsSize; i++)
  {
    ...
    printf("%d ", currentX);
    printf("%d ", currentY);
    printf("-------> %d ", abs(x - currentX) + abs(y - currentY));
    printf("\n");
    if (currentX != x && currentY != y)
      continue;

    if (abs(x - currentX) + abs(y - currentY) < ManhattanDistance)
    {
      ManhattanDistance = abs(x - currentX + y - currentY);
      ManhattanIndex = i;
    }
  }
